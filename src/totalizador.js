class Totalizador {
    ...
    calcularPrecioTotal(cantidad, precio, estado, categoria, peso_volumetrico, tipoCliente) {
        ...
        let costo_envio = parseFloat(this.calcularCostoEnvioTotal(cantidad, precio, peso_volumetrico, tipoCliente).split("$")[1]);
        let descuento_fijo = parseFloat(this.obtenerDescuentoFijoCliente(cantidad, precio, categoria, tipoCliente).split("$")[1]);
        let total = neto + impuesto + impuesto_adicional - descuento - descuento_adicional - descuento_fijo + costo_envio;
